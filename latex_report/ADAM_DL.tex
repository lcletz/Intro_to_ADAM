...
\par Sur l'échantillon de $10\,000$-observations, toutes les méthodes atteignent une précision de test identique de 99.77\%. Cependant, dans ce contexte de classes très déséquilibrées, la perte de test est un indicateur plus discriminant. Sur ce critère, Adam se démarque nettement avec une perte de 0.0057, significativement plus faible que celle des autres optimiseurs.
\par Sur un $100\,000$-échantillon, Adam se distingue comme le meilleur optimiseur avec une précision de 99.91\% et la plus faible perte de test (0.0033). Nous notons également que les performances d'Adagrad et RMSprop s'améliorent considérablement par rapport au plus petit échantillon, bien qu'elles restent légèrement en deçà de celles d'Adam.
...
\par On observe qu'Adagrad converge rapidement vers une faible erreur initiale, mais stagne ensuite, tandis qu'Adam continue à améliorer l'erreur au fil des \textit{epochs}. Adagrad converge le plus lentement, RMSprop un peu plus rapidement, et SGD converge rapidement dès l’initialisation à cause du choix du learning rate, ce qui atténue les phénomènes de divergence locale aperçus dans les autres méthodes.
...
\section{Annexes}
\label{annexes}
Afin de garantir la reproductibilité de nos résultats, l'ensemble du code utilisé pour les expériences de ce rapport est publiquement accessible sur notre dépôt GitHub\footnote{\url{https://github.com/lcletz/Intro_to_ADAM}}.
\subsection{Graphiques}

\begin{figure}[H]
\centering
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{images/training_error_n100k.png}
    \caption[Erreur d'entraînement (n=100k)]{Erreur d'entraînement ($n=100\,000$)}
    \label{fig:training_100k}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{images/training_loss_n10k.png}
    \caption[Perte d'entraînement (n=10k)]{Perte d'entraînement ($n=10\,000$)}
    \label{fig:loss_10k}
\end{subfigure}
\caption{Comparaison des métriques d'entraînement}
