...
\par Nous trouvons pour des échantillons de taille $n=10\,000$ et $n=100\,000$ les résultats suivants :
...
Graphiquement, ces observations se confirment à travers l'évolution des métriques d'entraînement et de test. La figure~\ref{fig:training_comparison} illustre l'évolution de l'erreur d'entraînement au cours des \textit{epochs}, révélant les vitesses de convergence distinctes de chaque optimiseur :

\begin{figure}[H]
\centering
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{images/training_error_n10k.png}
    \caption{Erreur d'entraînement}
...
L'erreur d'entraînement est la proportion de prédictions incorrectes sur l'ensemble d'entraînement, soit $1-$ la précision durant l'entraînement (\textit{training accuracy}), tandis que la perte mesure la qualité globale des prédictions. \\
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{Erreurs de test ($n=10\,000$)}
    \label{fig:test_10k}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{images/test_error_n100k.png}
    \caption{Erreurs de test ($n=100\,000$)}
    \label{fig:test_100k}
\end{subfigure}
...
\subsection*{Gradient non adaptatif}
...
Adagrad adapte le taux d’apprentissage en fonction de l’historique des
gradients :
\[v_t = v_{t-1} + g_t^2,
\]
...
RMSProp modifie Adagrad en utilisant une moyenne mobile exponentielle
...
La régression logistique est un type de régression dans les cas binaires. Sa fonction de perte est définit par 
$$
